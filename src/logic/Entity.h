...
/**
 * @brief Base model class for every object in the logic world.
 */
...
public:
     ...
     */
    EntityModel(std::size_t id, EntityType type, Vec2 position, Vec2 size);
...
/**
 * @brief Wall model used for indestructible and destructible arena blocks.
 */
class Wall final : public EntityModel {
public:
    /**
     * @brief Creates a wall model.
     * @param id Unique entity id.
     * @param type Wall or DestructibleBlock entity type.
     * @param position Center position.
     * @param size Wall size.
     */
    Wall(std::size_t id, EntityType type, Vec2 position, Vec2 size);
};

/**
 * @brief Bomb model placed by characters.
 */
...
public:
     ...
     */
    Bomb(std::size_t id, Vec2 position, Vec2 size);
};

/**
 * @brief Explosion model for one active explosion tile.
 */
class Explosion final : public EntityModel {
public:
    /**
     * @brief Creates an explosion with a drawing shape.
     * @param id Unique entity id.
     * @param position Center position.
     * @param size Explosion size.
     * @param explosionShape Shape used for drawing explosions.
     */
    Explosion(std::size_t id, Vec2 position, Vec2 size, ExplosionShape explosionShape);

    [[nodiscard]] std::optional<ExplosionShape> explosionShape() const override;

private:
    std::optional<ExplosionShape> explosionShapeKind{}; ///< Shape used by the explosion view.
};

/**
 * @brief Exit model used after all enemies are defeated.
 */
class Exit final : public EntityModel {
public:
    /**
     * @brief Creates a level exit.
     * @param id Unique entity id.
     * @param position Center position.
     * @param size Exit size.
     */
    Exit(std::size_t id, Vec2 position, Vec2 size);
};
